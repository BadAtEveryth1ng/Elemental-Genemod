...
def _get_event(
    ...
    other_clan = (
        (choice([c for c in game.clan.all_other_clans+[game.clan] if c.group_ID != main_cat.status.group_ID and (not other_cat or other_cat.status.group_ID == main_cat.status.group_ID)]) if game.clan.all_other_clans else None)
        if game.clan
        else None
    )
...
def _resolve_event(
    ...
    # FORMATTING
    replace_dict = {
        abbr: (str(c.name), choice(c.pronouns), c.phenotype.element) for abbr, c in involved_cats.items()
    }
    ...
    game.cur_events_list.append(
        EventInformation(event_string, ["relation", "interaction"], cat_ids, clan=involved_cats["m_c"].status.group_ID)
    )
