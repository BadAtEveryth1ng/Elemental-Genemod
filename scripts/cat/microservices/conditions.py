...
def get_ill(cat, illness_name, event_triggered=False, lethal=True, severity="default"):
    ...
    if cat.phenotype.element and illness_name in ELEMENT_BLOCK.get(cat.phenotype.element, []):
        print(
            f"WARNING: tried to give {cat.phenotype.element} cat {illness_name}")
        return
    ...
    if medicine_cats_can_cover_clan(cat.all_cats.values(), amount_per_med, cat.status.group_ID):
...
def get_injured(
    ...
    if name == "mangled tail" and ("NOTAIL" in cat.pelt.scars or cat.phenotype.bobtailnr > 0 and cat.phenotype.bobtailnr < 4):
        return
    if name == "torn ear" and ("NOEAR" in cat.pelt.scars or cat.phenotype.fold[0] == "Fd"):
        return
    ...
    if medicine_cats_can_cover_clan(
        cat.all_cats.values(), get_amount_cat_for_one_medic(), cat.status.group_ID
    ):
        duration = med_duration
    if severity != "minor":
        duration += randrange(-1, 1)
    ...
    if (
        ...
        if (
            ...
            and len(
                find_alive_cats_with_rank(cat, [CatRank.MEDICINE_CAT], working=True, clan=cat.status.group_ID)
            )
            != 0
        ):
            clan_herbs = {
    ...
    for illness in infectious_illnesses:
        ...
        if not random() * rate:
            ...
            game.cur_events_list.append(
                EventInformation(text, ["health"], cat_dict={"m_c": cat}, clan=cat.status.group_ID)
            )
...
def get_permanent_condition(cat, name, born_with=False, event_triggered=False, genetic=False, custom_reveal=None):
    if cat.dead:
        return False
    ...
    if "blind" in cat.permanent_condition and name == "failing eyesight":
        return False
    if "deaf" in cat.permanent_condition and name in ["partial hearing loss", "partial hearing loss in one ear", "deaf in one ear"]:
        return False
    if "partial hearing loss" in cat.permanent_condition and name in ["partial hearing loss in one ear"]:
        return
    if "deaf in one ear" in cat.permanent_condition and name in ["partial hearing loss in one ear"]:
        return
    ...
    if name == 'partially hairless' and cat.phenotype.length != 'fur-pointed':
